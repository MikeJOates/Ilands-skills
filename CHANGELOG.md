## 2026-08-18 — CHG-018 · Method v2.0.5 shipped (seats 17-21, komodo find, Adler kernel, Big Bruce naming)

- What changed: the_method_v2_0.md patched v2.0.4 → v2.0.5 (Mike's explicit go: "Looks good. Lock it in and update so it's our current version"). Orpheus seat 17 drill-selection — the weekly drill picks the change you least want to reverse. Charlene seats 18-20 — any apply touching a Hard Core Invariant drills its own rollback the same day; the witness gate lapses the MODE, never consent (default: two ritual cycles, lockfile-set); the failure log (date, attempt, hypothesis) joins Shelf D's admissible evidence. Cinder seat 21 the ask with teeth — the honour-ask 10 pins to the runner's first drill pass on their own ledger; the chain-pass to three rides on the read. komodo (find) — the Return Brief fires on the cycle boundary, reply or no reply. Adler (kernel) — floor receipts record the archive's sha256 on the ledger line (a path is a name, a hash is an object). Naming: creator credited as Big Bruce in four spots. Summary fix: forward-to-three now in the honour-ask summary paragraph (was section 11 + CHG-014 only — summary incomplete, not the ask, verified vs live v2.0.4). Lineage 16 → 21 seats. Same artifact slot serves v2.0.5 bytes (sha256 2f380762, verified by re-fetch). Skill copy synced (the-method/SKILL.md). DRILL-008 pressed the v2.0.4 reverse gear (prior hash e20a5323 from commit df51a54, matching live artifact, PASS). `ops-backup/` refreshed (lockfile v2.2, ledger CHG-018/DRILL-008).
- Why: six verified holes from five reviewers, all checked against the live bytes before merging; a naming correction; and the honour ask gets its unit — the income channel got teeth without losing the handshake.
- Status: ACTIVE (free door)
- Operator: Momo · Reviewers: Orpheus (17), Charlene (18-20), Cinder (21), komodo (find), Adler (kernel)
